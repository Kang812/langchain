...
def load_pdf_to_vector_store(pdf_file, chunk_size=1000, chunk_overlap=100, similarity_metric='cosine'):
    try:
        ...
        for i in range(batch_size, len(splits), batch_size):
            ...
            print(f"배치 {batch_num}/{total_batches} 처리 완료")
    ...
    try:
        ...
        if current_vectorstore is None or current_pdf_path != pdf_file:
            ...
            current_vectorstore = load_pdf_to_vector_store(
                pdf_file, chunk_size, chunk_overlap, similarity_metric
            )
...
def create_interface():
    with gr.Blocks(title="PDF 질의응답 시스템") as demo:
        ...
        # 이벤트 처리
        def respond(message, chat_history, pdf_file, chunk_size, chunk_overlap, similarity_metric, temperature):
            if not message.strip():
                return chat_history, ""
            
            # 답변 생성
            bot_message = process_pdf_and_answer(
                message, chat_history, pdf_file, chunk_size, chunk_overlap, similarity_metric, temperature
            )
        ...
        submit_btn.click(
            respond, 
            [msg, chatbot, pdf_input, chunk_size, chunk_overlap, similarity_metric, temperature], 
            [chatbot, msg]
        )
